...
export const ACCOUNT_REGISTRATION_CHANGED =
  'pjsip/ACCOUNT_REGISTRATION_CHANGED';
  ...
  return async function(dispatch, getState) {
    ...
    endpoint.on('call_received', call => {
      ...
      RNCallKit.displayIncomingCall(
        uuids[call.getCallId()],
        call.getRemoteFormattedNumber(),
      );
    });
    endpoint.on('call_changed', call => {
      if (
        call.getId() === incomingCall &&
        call.getState() != 'PJSIP_INV_STATE_INCOMING'
      ) {
        incomingCall = null;
    ...
    if (!account) {
      dispatch(
        Navigation.goTo({
          name: 'call',
          call: Promise.reject(
            'At least one account should be available to make a call.',
          ),
        }),
...
export function changeNetworkSettings(configuration) {
  return async (dispatch, getState) => {
    ...
    const settings = await endpoint.changeServiceConfiguration(
...
export function onCallReceived(call) {
  return async function(dispatch, getState) {
    ...
    if (
      state.navigation.current.name != 'call' &&
      state.pjsip.appState == 'active'
    ) {
      dispatch(Navigation.goTo({name: 'call', call})); // TODO: Move to another place
    }
